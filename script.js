document.addEventListener("DOMContentLoaded", () => {
    ...
    let musicPlaying = false; // تم النقل هنا لضمان الوصول للمتغير بشكل صحيح
    ...
    window.openInvitation = function() {
        ...
        // التعديل: تشغيل الموسيقى فوراً عند تفاعل المستخدم (الضغط) ليتوافق مع أجهزة آيفون
        audio.volume = 0.35;
        audio.play().then(() => {
            document.getElementById('musicBars').classList.remove('paused');
            musicPlaying = true;
        }).catch(() => {
            // Autoplay blocked (mobile) — bars stay paused, user taps FAB
    ...
    function startCountdown() {
        function tick() {
            const target = new Date('2026-05-27T18:00:00').getTime();
            const now    = Date.now();
    ...
    if (rsvpForm) {
        rsvpForm.addEventListener('submit', (e) => {
            ...
            const attendingText = attending
                ? (attending.value === 'yes' ? 'Joyfully Accept' : 'Regretfully Decline')
                : 'Not specified';

            const text =
                `Wedding RSVP - Mohamed & Israa\n\n` +
                `Name: ${name}\n` +
                `Attendance: ${attendingText}\n` +
                `Guests: ${guests}\n` +
                (diet    ? `Dietary Requirements: ${diet}\n` : '') +
                (song    ? `Song Request: ${song}\n` : '') +
                (message ? `Note: ${message}\n` : '') +
                `\nMay 27, 2026 - Hayah Hall`;

            const phone   = '201551881613';
...
(function spawnHearts() {
    setInterval(() => {
        ...
        // التعديل: إضافة \uFE0E لإجبار نظام آيفون على التعامل معه كنص ليتلون بالأبيض
        h.innerText = '\u2764\uFE0E';
        h.style.cssText =
            'left:' + (Math.random() * 100) + 'vw;' +
